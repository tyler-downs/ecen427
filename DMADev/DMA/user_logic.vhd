...
architecture IMP of user_logic is
  ...
  --USER signal declarations added here, as needed for user logic
	...
	signal destAddress : std_logic_vector(31 downto 0);
	signal transferLength : unsigned(31 downto 0);
	signal currentTransferLength : unsigned(31 downto 0) := (others => '0');
	signal userGo : std_logic;
	signal allowTransfer : std_logic;
	
	type USER_STATE_TYPE is (WAIT_FOR_GO_ST, READ_ST, WRITE_ST);
	signal userState : USER_STATE_TYPE;
  ------------------------------------------
  ...
  signal slv_reg_write_sel              : std_logic_vector(6 downto 0);
  signal slv_reg_read_sel               : std_logic_vector(6 downto 0);
...
begin
  ...
  --USER logic implementation added here
	...
	begin
		if Bus2IP_Clk'event and Bus2IP_Clk = '1' then
			...
			userGo <= userGo;
			currentTransferLength <= currentTransferLength;
			mst_cntl_rd_req <= '0';
			mst_cntl_wr_req <= '0';
			allowTransfer <= allowTransfer;
			
			if Bus2IP_Resetn = '0' then
				currentTransferLength <= (others => '0');
				userState <= WAIT_FOR_GO_ST;
				allowTransfer <= '0';
			else
				case userState is
					when WAIT_FOR_GO_ST =>
							sourceAddress <= slv_reg0;
							destAddress <= slv_reg1;
							transferLength <= slv_reg2;
							currentTransferLength <= (others => '0');
							userGo <= slv_reg3(0);
					...
					when READ_ST =>
							...
							if (Bus2IP_Mst_Cmplt = '1') then
									userState <= WRITE_ST;
									currentTransferLength <= currentTransferLength + 1;
							end if;
					...
					when WRITE_ST =>
							...
							allowTransfer <= '1';
							if (Bus2IP_Mst_Cmplt = '1') then
									if (currentTransferLength >= transferLength) then
										userState <= WAIT_FOR_GO_ST;
									else
										userState <= READ_ST;	
									end if;
  ...
  -- 
  ------------------------------------------
  slv_reg_write_sel <= Bus2IP_WrCE(6 downto 0);
  slv_reg_read_sel  <= Bus2IP_RdCE(6 downto 0);
  ...
  -- 
  ------------------------------------------
  mst_reg_write_req <= Bus2IP_WrCE(7) or Bus2IP_WrCE(8) or Bus2IP_WrCE(9) or Bus2IP_WrCE(10);
  mst_reg_read_req  <= Bus2IP_RdCE(7) or Bus2IP_RdCE(8) or Bus2IP_RdCE(9) or Bus2IP_RdCE(10);
  mst_reg_write_sel <= Bus2IP_WrCE(10 downto 7);
  mst_reg_read_sel  <= Bus2IP_RdCE(10 downto 7);
  mst_write_ack     <= mst_reg_write_req;
  mst_read_ack      <= mst_reg_read_req;

  -- rip control bits from master model registers
-- mst_cntl_rd_req   <= mst_reg(0)(0); --TAKING THIS OVER
 ...
 -- mst_ip2bus_be     <= mst_reg(9) & mst_reg(8);
  ...
  begin
    ...
    if ( Bus2IP_Clk'event and Bus2IP_Clk = '1' ) then
      if ( Bus2IP_Resetn = '0' or mst_cmd_sm_clr_go = '1') then --CHECK OUR GO SIGNAL
        mst_go   <= '0';
            ...
            Bus2IP_Data((GO_BYTE_LANE-(GO_BYTE_LANE/BE_WIDTH)*BE_WIDTH)*8+7   downto
                         (GO_BYTE_LANE-(GO_BYTE_LANE/BE_WIDTH)*BE_WIDTH)*8)= GO_DATA_KEY ) and allowTransfer = '1') then
        mst_go   <= '1';
      else
        null;
      end if;
    ...
    if ( Bus2IP_Clk'event and Bus2IP_Clk = '1' ) then
      if ( Bus2IP_Resetn = '0' ) then
        ...
        case mst_cmd_sm_state is
          ...
          when CMD_RUN =>
            if ( Bus2IP_Mst_CmdAck = '1' and Bus2IP_Mst_Cmplt = '0' ) then --CHECK FOR OUR BIT TOO?
